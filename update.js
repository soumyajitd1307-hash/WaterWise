...
html = html.replace(/font-family:\s*'DM Sans',\s*sans-serif;/g, "font-family: 'Inter', sans-serif;");
html = html.replace(/font-family:\s*'Playfair Display',\s*serif;/g, "font-family: 'Outfit', sans-serif;");
html = html.replace(/family:'DM Sans'/g, "family:'Inter'");
html = html.replace(/font-family="DM Sans"/g, 'font-family="Inter"');
html = html.replace(/font-family:'DM Sans',sans-serif;/g, "font-family:'Inter',sans-serif;");
html = html.replace(/font-family:'Playfair Display',serif;/g, "font-family:'Outfit',sans-serif;");

// 2. Change textContent to innerHTML for elements that will receive FontAwesome icons in JS
const textContentToInnerHTML = [
  's.textContent=label;',
...
for (const target of textContentToInnerHTML) {
  html = html.split(target).join(target.replace('textContent', 'innerHTML'));
}

// Remove emojis from Chart.js labels (Canvas text doesn't support HTML tags easily)
html = html.replace(/'Shower 🚿'/g, "'Shower'");
html = html.replace(/'Toilet 🚽'/g, "'Toilet'");
html = html.replace(/'Handwash 🧼'/g, "'Handwash'");
html = html.replace(/'Kitchen 🍽️'/g, "'Kitchen'");
html = html.replace(/'Laundry 👕'/g, "'Laundry'");
html = html.replace(/'Outdoor 🌿'/g, "'Outdoor'");
...
for (const [emoji, tag] of Object.entries(map)) {
...
console.log("Done updating fonts and emojis to icons!");
